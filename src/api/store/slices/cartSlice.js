...
const mergeCartItems = (existingItems, newItems) => {
  ...
  newItems.forEach((newItem) => {
    ...
    const existingItemIndex = mergedItems.findIndex(
      (item) => item.product.id === newItem.product_id && item.attributeKey === attributeKey,
    )
  ...
  return {
    ...
    total: items
      .reduce((sum, item) => {
        ...
        const finalPrice = campaignDiscount ? price - (price * campaignDiscount) / 100 : price
...
export const removeCartItem = createAsyncThunk(
  ...
  async ({ productId, cartId, attributeKey }, { getState, rejectWithValue, dispatch }) => {
    try {
      ...
      // If cartId is not available, try to find it in the current cart items
      let itemCartId = cartId
      if (!itemCartId) {
        const cartItems = state.cart.items
        const item = cartItems.find(
          (item) =>
            item.product.id === productId &&
            (item.attributeKey === attributeKey || (!item.attributeKey && !attributeKey)),
        )
        if (item) {
          itemCartId = item.id
        }
      ...
      // Only proceed with the API call if we have a valid cartId
      if (itemCartId) {
        ...
        // If we still don't have a cartId, just remove it locally
...
const deduplicateCartItems = (items) => {
  return [...new Map(items.map((item) => [item.product_id + (item.attributeKey || ""), item])).values()]
}
  ...
  reducers: {
    ...
    addToCart: {
      reducer: (state, action) => {
        ...
        const existingItemIndex = state.items.findIndex(
          (item) => item.product.id === product.id && item.attributeKey === attributeKey,
        )

        if (existingItemIndex !== -1) {
          // Update existing item quantity and attributes
          state.items[existingItemIndex].quantity = Math.max(1, state.items[existingItemIndex].quantity + quantity)
      ...
      prepare: (payload) => {
        ...
        const sanitizedQuantity = Math.max(1, Number.parseInt(quantity, 10) || 1)
        ...
        if (product.attributes && product.attributes.length > 0) {
          ...
          const missingAttributes = product.attributes.filter(
            (attr) => !selectedAttributes.some((sel) => sel.attribute_name === attr.name),
          )
    ...
    updateQuantity: (state, action) => {
      ...
      const itemIndex = state.items.findIndex(
        (item) => item.product.id === productId && item.attributeKey === attributeKey,
      )

      if (itemIndex !== -1) {
        const updatedQuantity = Math.max(1, Number.parseInt(quantity, 10) || 1)
...
// Export Actions
export const { addToCart, removeFromCart, updateQuantity, clearCart, setActiveCart, updateCartFromServer, clearError } =
  cartSlice.actions
